...
function valueType(param, paramTheSecond) {
  if (param === paramTheSecond) return "Exactly the same"
  if (param == paramTheSecond) return "Same value, different types"
...
function promiseCatcher(parameter) {
  parameter.then(function (result) {
    theAnswer = result
  })
}